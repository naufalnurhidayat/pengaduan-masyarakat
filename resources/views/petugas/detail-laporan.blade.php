        ...
        <div class="col-md8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="mt-2">
                        <a href="{{ url('/petugas/laporan') }}" class="btn btn-primary">Kembali</a>
                        @if($pengaduan->status == '0')
                            <form action="{{ url('/petugas/laporan/proses/' . $pengaduan->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('patch')
                                <button type="submit" class="btn btn-warning">Proses Laporan Ini</button>
                            </form>
                            <a href="{{ url('/petugas/laporan/tanggapan/' . $pengaduan->id ) }}" class="btn btn-success">Tanggapi dan Selesai</a>
                        @elseif($pengaduan->status == 'proses')
                            <a href="{{ url('/petugas/laporan/tanggapan/' . $pengaduan->id ) }}" class="btn btn-success">Tanggapi dan Selesai</a>
                        @endif
                    </div>
